...
INSTRUMENT_CHOICES = [(i, GM_INSTRUMENTS[i]) for i in range(len(GM_INSTRUMENTS))]

class InstrumentSelectionForm(forms.Form):
    left_instrument = forms.ChoiceField(choices=INSTRUMENT_CHOICES, required=True)
    right_instrument = forms.ChoiceField(choices=INSTRUMENT_CHOICES, required=True)